...
app.post('/todo', function (req, res) {
    ...
    mc.query("INSERT INTO tasks SET ? ", { task: task }, function (error, results, fields) {
        if (error) throw error;
        return res.send({ error: false, data: results, message: 'New task has been created successfully.',id:results.insertId });
    });
});
...
app.all("*", function (req, res) {
    return res.status(404).send({status : false,message :"Invalid Request Path"})
});
